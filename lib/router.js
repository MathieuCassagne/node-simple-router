...
(function() {
  ...
  Router = function(options) {
    var _make_raw_request_wrapper, default_options, dispatch, domain, escaped_favicon, escaped_icons_png, escaped_pixel_gif, fs, http, mime_types, net, nsr_sessions, padString, path_tools, querystring, spawn, thousandSep, urlparse, uuid, _bodyparser, _extend, _getSessionHandler, _isEmpty, _make_request_wrapper, _multipartparser, _parsePattern, _pushRoute;
    if (options == null) {
      options = {};
    }
    ...
    padString = function(stri, quantity, direction, padchar) {
          ...
          return req.on('end', function() {
            ...
            if (mp_index === -1) {
              ...
              for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                obj = _ref[_i];
                if (obj.fileName) {
                  req.fileName = obj.fileName;
                }
                if (obj.fileLen) {
                  req.fileLen = obj.fileLen;
                }
                if (obj.fileData) {
                  req.fileData = obj.fileData;
                }
                if (obj.fileType) {
                  req.fileType = obj.fileType;
                }
              }
            }
    ...
    // Simpler wrapper dedicated to binary put only
    _make_raw_request_wrapper = function(cb) {
      var wrapper;
      wrapper = function(req, res) {
        var contentType, e, mp_index;
    ...
    dispatch = function(req, res) {
      var final_dispatch, nsr_sid;
      final_dispatch = function(req, res) {
        var args, home_page, homes, index, m, method, param, parsed, pathname, route, selected_method, send_static_home, _i, _j, _len, _len1, _ref;
        ...
        for (_i = 0, _len = selected_method.length; _i < _len; _i++) {
          route = selected_method[_i];
        ...
        if (pathname === "/") {
          homes = (function() {
            ...
            _ref1 = dispatch.default_home;
            _results = [];
            for (_k = 0, _len2 = _ref1.length; _k < _len2; _k++) {
              home_page = _ref1[_k];
              _results.push("" + dispatch.static_route + path_tools.sep + home_page);
            }
            return _results;
          })();
          send_static_home = function(candidates) {
            if (candidates.length === 0) {
              ...
              return fs.exists(candidates[0], function(exists) {
                if (exists) {
                  return dispatch["static"]("/" + (path_tools.basename(candidates[0])), req, res);
                } else {
                  return send_static_home(candidates.slice(1));
                }
              });
            }
          };
    ...
    _bodyparser = function(body, contentType) {
      ...
      try {
        ...
        if(typeof parsedBody !== 'object') {
          throw 'JSON parse failed for body as string is not valid JSON';
        }
        return parsedBody;
      } catch (_error) {
        e = _error;
    ...
    dispatch.getRemoteAddress = function(req) {
      var ip;
      return ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress || req.socket.remoteAddress || req.connection.socket.remoteAddress;
    };
    dispatch.get_route_handler = function(path, method) {
      var chosen, key, object, objects, _i, _j, _len, _len1, _ref;
      if (!method) {
        _ref = dispatch.routes;
        for (key in _ref) {
          objects = _ref[key];
          for (_i = 0, _len = objects.length; _i < _len; _i++) {
            object = objects[_i];
            if (path.match(object.pattern)) {
              return {
                method: key,
                handler_obj: object
              };
    ...
    dispatch.get_icon = function(x, y) {
      var context, offsetX, offsetY, template;
    ...
    dispatch.stock_icons = {
      directory: function() {
        return dispatch.get_icon(13, 3);
      },
      file: function() {
        return dispatch.get_icon(20, 3);
      }
    };
    dispatch.getUUID = function() {
      return uuid();
    };
    dispatch.cookie2obj = function(cookie_str) {
      var cookiePairs, eqIndex, key, pair, ret, value, _i, _len;
      ret = {};
      cookiePairs = (function() {
        var _i, _len, _ref, _results;
        _ref = cookie_str.split(/;/);
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          pair = _ref[_i];
          _results.push(pair.trim());
        }
        return _results;
      })();
      for (_i = 0, _len = cookiePairs.length; _i < _len; _i++) {
        pair = cookiePairs[_i];
    ...
    dispatch.obj2cookie = function(obj) {
      var key, val;
      return ((function() {
        var _results;
        _results = [];
    ...
    dispatch.getCookie = function(request, cookie_name) {
      var obj, ret;
      ret = {};
    ...
    dispatch.setCookie = function(response, cookie_obj, max_age) {
      var arr, key, value;
      arr = [];
      for (key in cookie_obj) {
        value = cookie_obj[key];
        arr.push("" + key + "=" + value + (max_age ? '; max-age=' + max_age : ''));
      }
      if (arr.length !== 0) {
        response.setHeader('Set-Cookie', arr);
      }
      return dispatch.obj2cookie(cookie_obj);
    };
    dispatch.memory_store = function(request, opcode, sessobj, cb) {
      var sid, sid_obj, _ref;
      if (opcode == null) {
        opcode = 'get';
      }
      if (sessobj == null) {
      ...
      switch (opcode) {
        case 'set':
          ...
          break;
        case 'get':
          if ((_ref = !sid, __indexOf.call(nsr_sessions, _ref) >= 0) || (!nsr_sessions[sid])) {
            nsr_sessions[sid] = sessobj;
          }
          break;
        default:
      ...
      return cb(request.nsr_session);
    };
    dispatch.text_store = function(request, opcode, sessobj, cb) {
      var filename, get_func, set_func, sid, sid_obj;
      if (opcode == null) {
        opcode = 'get';
      }
      if (sessobj == null) {
      ...
      set_func = function() {
        return fs.exists(path_tools.dirname(filename), function(exists) {
          var file;
          if (!exists) {
            return fs.mkdir(path_tools.dirname(filename), function(err) {
              if (err) {
                return cb(err);
              } else {
                return set_func();
    ...
    dispatch.getSession = function(request, cb) {
      if (cb == null) {
        cb = (function(id) {
          return id;
        });
      ...
      return _getSessionHandler()(request, 'get', {}, cb);
    };
    dispatch.setSession = function(request, sessionObj, cb) {
      if (cb == null) {
        cb = (function(id) {
          return id;
        });
      ...
      return _getSessionHandler()(request, 'set', sessionObj, cb);
    };
    dispatch.updateSession = function(request, sessionObj, cb) {
      if (cb == null) {
        cb = (function(id) {
          return id;
        });
      ...
      return dispatch.getSession(request, function(curr_session_obj) {
        if (curr_session_obj) {
          return dispatch.setSession(request, _extend(curr_session_obj, sessionObj), cb);
        }
      });
    };
    dispatch.setSessionHandler = function(func_name) {
      var num;
    ...
    dispatch.proxy_pass = function(url, response) {
      ...
      try {
        ...
        e = _error;
        return dispatch._500(null, response, url, e.message);
      }
    };
    dispatch.dir_list = function(fpath, path) {
      ...
      return p.then(function(files) {
        ...
        files.forEach(function(file, index) {
          return fs.stat("" + fpath + path_tools.sep + file, function(err, stat) {
            if (err) {
              return d.reject(err);
            } else {
              stats[index] = stat;
        ...
        if (dispatch.logging) {
          return dispatch.log("Error reading directory: " + err.message);
        }
      });
    };
    dispatch.gallery = function(fpath, path, res, list_func) {
      var context;
      if (list_func == null) {
      ...
      return list_func(fpath, path).then(function(arrs) {
        var file, files;
        files = arrs[0];
        context.files = (function() {
          ...
          for (_i = 0, _len = files.length; _i < _len; _i++) {
            file = files[_i];
            if ((_ref = mime_types[path_tools.extname(file.toLowerCase())]) != null ? _ref.match(/image/) : void 0) {
              _results.push({
                url: "" + path + "/" + (querystring.escape(file))
              });
            }
          }
          return _results;
        ...
        if (context.files.length !== 0) {
          ...
          return res.end(dispatch.render_template(dispatch._gallery_template, context));
        } else {
          return dispatch.static_directory(fpath, path, res, list_func);
        }
      }, function(err) {
        return dispatch._404(null, res, path);
      });
    };
    dispatch.directory = function(fpath, path, res, list_func) {
      if (list_func == null) {
        list_func = dispatch.dir_list;
      }
      return dispatch.static_directory(fpath, path, res, list_func);
    };
    dispatch.static_directory = function(fpath, path, res, list_func) {
      var context;
      if (list_func == null) {
        list_func = dispatch.dir_list;
      }
      ...
      return list_func(fpath, path).then(function(arrs) {
        var file, files, index, obj, resp, stats, _i, _len;
        ...
        for (index = _i = 0, _len = files.length; _i < _len; index = ++_i) {
          ...
          obj["class"] = stats[index].isDirectory() ? "directory" : "file";
          obj.order = stats[index].isDirectory() ? 1 : 0;
          obj.icon = stats[index].isDirectory() ? dispatch.stock_icons.directory() : dispatch.stock_icons.file();
          context.cwd_contents.push(obj);
        }
        context.cwd_contents.sort(function(o1, o2) {
    ...
    dispatch.postraw = function(pattern, callback) {
      return _pushRoute(pattern, _make_raw_request_wrapper(callback), 'post');
    };
    dispatch.put = function(pattern, callback) {
      return _pushRoute(pattern, _make_request_wrapper(callback), 'put');
    };
    dispatch.putraw = function(pattern, callback) {
      return _pushRoute(pattern, _make_raw_request_wrapper(callback), 'put');
    };
    dispatch.patch = function(pattern, callback) {
      return _pushRoute(pattern, _make_request_wrapper(callback), 'patch');
    };
    ...
    dispatch.head = function(pattern, callback) {
      return _pushRoute(pattern, callback, 'head');
    };
    dispatch.options = function(pattern, callback) {
      return _pushRoute(pattern, callback, 'options');
    };
    dispatch.any = function(pattern, callback) {
      return _pushRoute(pattern, _make_request_wrapper(callback), 'any');
    };
    dispatch._404 = function(req, res, path) {
      ...
      default_resp = "<h2>404 - Resource " + path + " not found at this server</h2>\n<hr/><h3>Served by " + dispatch.served_by + " v" + dispatch.version + "</h3>\n<p style=\"text-align: center;\"><button onclick='history.back();'>Back</button></p>";
      return fs.exists("" + dispatch.static_route + path_tools.sep + "404.html", function(exists) {
        if (!exists) {
          return res.end(default_resp);
        } else {
          return fs.readFile("" + dispatch.static_route + path_tools.sep + "404.html", {
            encoding: 'utf8'
          }, function(err, data) {
            if (err) {
              return res.end(default_resp);
    ...
    dispatch.render_template = function(template_string, context, keep_tokens) {
      var dont_encode, e, full_text, html_encode, index, item, k, key, new_str, property, replacement, ret_arr, section, section_inner_text, section_pattern, section_pattern_global, section_tokens, section_type, sections, stripped_string, text, text_token, text_tokens, token, token_obj, value, variable_pattern, variable_pattern_global, variable_tokens, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m;
      if (keep_tokens == null) {
        keep_tokens = false;
      }
      "Naive regex based implementation of mustache.js spec";
      html_encode = function(stri) {
        return String(stri).replace(/[<>&'\/"]/g, function(m) {
          return {
            "<": "&lt;",
            ">": "&gt;",
            "&": "&amp;",
      ...
      for (index = _j = 0, _len1 = text_tokens.length; _j < _len1; index = ++_j) {
        text_token = text_tokens[index];
        variable_tokens = text_token.match(variable_pattern_global);
        token_obj = {};
        if (variable_tokens) {
          for (_k = 0, _len2 = variable_tokens.length; _k < _len2; _k++) {
            token = variable_tokens[_k];
        ...
        for (key in token_obj) {
          ...
          if (replacement) {
            new_str = new_str.replace(new RegExp(token_obj[key].token, 'g'), token_obj[key].dont_encode ? replacement : html_encode(replacement));
          }
        }
        if (!keep_tokens) {
      ...
      ret_arr = [];
      for (index = _l = 0, _len3 = text_tokens.length; _l < _len3; index = ++_l) {
        ...
        if (section_tokens[index]) {
          ...
          section_type = section_tokens[index][1] === "#" ? true : false;
          section_inner_text = section_tokens[index][3];
          if (section_type === false) {
            if (!property) {
              ret_arr.push(dispatch.render_template(section_inner_text, context));
            }
          } else {
            if (((property != null ? property.length : void 0) != null) && ((property != null ? property.constructor.name : void 0) !== "String")) {
              for (_m = 0, _len4 = property.length; _m < _len4; _m++) {
                item = property[_m];
    ...
    dispatch.render_template_file = function(file_name, context, cb, keep_tokens) {
      if (cb == null) {
        cb = (function(exists, text) {
          return [exists, text];
        });
      }
      if (keep_tokens == null) {
      ...
      return fs.exists(file_name, function(exists) {
        var ss, tf;
        if (!exists) {
        ...
        ss.on('finish', function() {
          ss.transform(function(text) {
            return dispatch.render_template(text, context, keep_tokens);
          });
          return cb(exists, '' + ss);
        });
        return tf.pipe(ss);
